...
class CurrentHistoryTests(unittest.TestCase):
    def test_current_and_gap_batches_validate(self):
        ...
        self.assertEqual(len(current), 339)
        self.assertEqual(len(gaps), 22)
        self.assertTrue(all(row["date"] <= "2026-09-14" for row in current))
    ...
    def test_unified_catalog_has_no_cross_batch_duplicates(self):
        ...
        self.assertEqual(len(rows), 41018)
        self.assertEqual(len({(r["league"], r["date"], r["home_team"], r["away_team"]) for r in rows}), 41018)
    ...
    def test_latest_sourced_result_batches(self):
        ...
        update = json.loads((ROOT / "reports/current_season_update_20260914.json").read_text())
